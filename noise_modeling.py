...
class PulsarNoiseModelPipeline:
    def __init__(self, **kwargs):
        ...
        for comp in ['RN', 'DM', 'SV', 'SW']:
            val = self.kargs.get(f'{comp}_coeff')
            if val is not None:
                self.user_bins[comp] = val
        self.models_to_run = self.parse_models()
        self.total_steps = 0
        for model in self.models_to_run:
            comps = self.get_model_components(model)
            comp_names = [c['name'] for c in comps]
            if all(c in self.user_bins for c in comp_names):
                self.total_steps += 1  # only step 2 will run
            else:
                self.total_steps += 2  # step 1 + step 2
    ...
    def get_bin_number(self, comp_name, step, bin_numbers=None):
        ...
        # 1. If bin_numbers is provided (step 2) and contains the component, use it.
        if step == 2 and (bin_numbers and comp_name in bin_numbers):
            return bin_numbers[comp_name]
        # 2. If the user provided a value for this component, use it.
        if comp_name in self.user_bins:
            return self.user_bins[comp_name]
        # 3. Fall back to default maximum bin.
        if step == 1:
            return self.kargs.get(f'{comp_name}_bin')
        else:  # step 2 without bin_numbers: should not happen, but safe fallback
            return self.kargs.get(f'{comp_name}_bin')
    ...
    def build_model_string(self, components, step, bin_numbers=None, log=False, fitwn=False):
        ...
        for comp in components:
            ...
            if step == 1:
                if comp["name"] in self.user_bins:
                    # User-specified:  no dropbin
                    if comp["name"] == "RN":
                    ...
                    # Not user-specified: include dropbin
                    if comp["name"] == "RN":
    ...
    def extract_bin_numbers(self, output_dir, model_name):
        ...
        try:
            ...
            for comp in components:
                if comp["name"] in self.user_bins:
                    bin_comps[comp["name"]] = self.user_bins[comp["name"]]
                    continue
    ...
    def process_single_model(self, model_name, progress_queue):
        ...
        user_specified_comps = set(comp_names) & set(self.user_bins.keys())
        all_specified = (len(user_specified_comps) == len(comp_names))
        if all_specified:
            self.logger.info(f"All components of model {model_name} have user-specified bin numbers. Skipping step 1.")
            # Use user bins directly for step 2
            bin_numbers = {c: self.user_bins[c] for c in comp_names}
            ...
            if self.run_enterprise_analysis(model_name, 2, bin_numbers):
                if self.run_plot_results(model_name, 2, bin_numbers):
                    progress_queue.put(1)  # only one step for this model
            self.logger.info(f"Complete processing model: {model_name}")
            return
        # Not all specified: proceed with step 1
        self.logger.info(f"Model {model_name}: Step 1 - bin number selection "
                         f"(user-specified components: {user_specified_comps})")
        if self.run_enterprise_analysis(model_name, 1):
            if self.run_plot_results(model_name, 1):
                progress_queue.put(1)
        # Extract bin numbers from step 1 results, but preserve user-specified ones
        extracted_bins = self.extract_bin_numbers(f"{self.kargs['datadir']}/{model_name}/", model_name)
        # Merge: user-specified bins override extracted ones
        bin_numbers = {**extracted_bins, **{c: self.user_bins[c] for c in user_specified_comps}}
        self.logger.info(f"The bin numbers of model {model_name} are: {bin_numbers}")
        self.logger.info(f"Model {model_name}: Step?2 - model selection with fixed bin number")
        if self.run_enterprise_analysis(model_name, 2, bin_numbers):
            if self.run_plot_results(model_name, 2, bin_numbers):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rnc", "--RN-coeff", type=int, dest='RN_coeff', default=None,
                        help="The number of coefficients for RN.")
    parser.add_argument("--dmc", "--DM-coeff", type=int, dest='DM_coeff', default=None,
                        help="The number of coefficients for DM.")
    parser.add_argument("--svc", "--SV-coeff", type=int, dest='SV_coeff', default=None,
                        help="The number of coefficients for SV.")
    parser.add_argument("--swc", "--SW-coeff", type=int, dest='SW_coeff', default=None,
                        help="The number of coefficients for SW.")
